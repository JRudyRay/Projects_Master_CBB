...
linear_regression = function(values1, values2) {
    ...
    mod <- lm(values2~values1, data.frame(values1, values2))
    mod <- summary(mod)
...
calculate_trait_at_internal_node = function(trait_child1, trait_child2, branch_length_to_child1, branch_length_to_child2) {
    ...
    x1 <- trait_child1*(branch_length_to_child2/(branch_length_to_child1+branch_length_to_child2))
    x2 <- trait_child2*(branch_length_to_child1/(branch_length_to_child1+branch_length_to_child2))
    trait_at_internal_node <- x1 + x2
...
calculate_contrast = function(trait_child1, trait_child2, branch_length_to_child1, branch_length_to_child2) {
    ...
    #   branch_length_to_child1: (corrected) branch length leading to first child (numeric)
    #   branch_length_to_child2: (corrected) branch length leading to second child (numeric)
    contrast <- (trait_child1 - trait_child2)/(branch_length_to_child1+branch_length_to_child2)^0.5
...
calculate_corrected_branch_length = function(node, child1, child2, corrected_branch_lengths) {
    ...
    corrected_length <- ((t1*t2)/(t1+t2))+corrected_branch_lengths[node]
...
get_contrasts_in_subtree = function(node, tree, description) {
    ...
    # for the child nodes, saving the updated description.
      ...
      # get children of the children, check if they are null or not, to determine next step
      child <- get_node_children(child1, tree)
      chil11 <- child$child1
      chil12 <- child$child2
      
      if( is.null(chil11) == TRUE){
        ...
        dd1 <- calculate_trait_and_contrast(child1, child2, description$traits[,1], description$corrected_branch_lengths)
        
        description$traits[node,1] <- dd1$trait_value
        description$normalized_contrasts[node,1] <- dd1$contrast
        
        dd2 <- calculate_trait_and_contrast(child1, child2, description$traits[,2], description$corrected_branch_lengths)
        
        description$traits[node,2] <- dd2$trait_value
        description$normalized_contrasts[node,2] <- dd2$contrast
      }
      
      
      
      else{
        description$corrected_branch_lengths[child1] <- calculate_corrected_branch_length(child1, chil11, chil12, description$corrected_branch_lengths)
        
        # doing it for the children of the children
        dd1 <- calculate_trait_and_contrast(chil11, chil12, description$traits[,1], description$corrected_branch_lengths)
        
        description$traits[child1,1] <- dd1$trait_value
        description$normalized_contrasts[child1,1] <- dd1$contrast
        
        dd2 <- calculate_trait_and_contrast(chil11, chil12, description$traits[,2], description$corrected_branch_lengths)
        
        description$traits[child1,2] <- dd2$trait_value
        description$normalized_contrasts[child1,2] <- dd2$contrast
        
        
        # now do the same for the other child of the node
        chil <- get_node_children(child2, tree)
        chil21 <- chil$child1
        chil22 <- chil$child2
        
        description$corrected_branch_lengths[child2] <- calculate_corrected_branch_length(child2, chil21, chil22, description$corrected_branch_lengths)
        
        dd1 <- calculate_trait_and_contrast(chil21, chil22, description$traits[,1], description$corrected_branch_lengths)
        
        description$traits[child2,1] <- dd1$trait_value
        description$normalized_contrasts[child2,1] <- dd1$contrast
        
        
        dd2 <- calculate_trait_and_contrast(chil21, chil22, description$traits[,2], description$corrected_branch_lengths)
        
        description$traits[child2,2] <- dd2$trait_value
        description$normalized_contrasts[child2,2] <- dd2$contrast
        
        
        
        ## now do the whole thing for the node
...
get_trait_correlation = function(newick_tree, traits_at_tip) {	
    ...
    raw_data_regression <- linear_regression(updated_description$traits[1:4,1], updated_description$traits[1:4,2])
