...
get_contrasts_in_subtree = function(node, tree, description) {
    ...
    # Update the branch length leading to the given node, save it in the description
      ...
      updated_description$corrected_branch_lengths[node] <- calculate_corrected_branch_length(node, child1, child2, updated_description$corrected_branch_lengths)
      
      
    # Compute the trait values and normalized contrasts for both traits, save the values in the description
      
      dd1 <- calculate_trait_and_contrast(child1, child2, updated_description$traits[,1], updated_description$corrected_branch_lengths)

      updated_description$traits[node,1] <- dd1$trait_value
      updated_description$normalized_contrasts[node,1] <- dd1$contrast

      dd2 <- calculate_trait_and_contrast(child1, child2, updated_description$traits[,2], updated_description$corrected_branch_lengths )

      updated_description$traits[node,2] <- dd2$trait_value
      updated_description$normalized_contrasts[node,2] <- dd2$contrast
